...
public class MoneyCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        switch (args.length) {
            case 0:
                player.sendMessage("§4§lGrieferWeit §8» §7Du hast zurzeit §e" + CoinSystem.getInstance().getCoinAPI().getCoins(player.getUniqueId()) + "€");
                break;
            case 3:
                ...
                switch (args[0].toLowerCase()) {
                    case "add":
                        CoinSystem.getInstance().getCoinAPI().addCoins(Bukkit.getPlayer(args[1]).getUniqueId(), Integer.parseInt(args[2]));
                        player.sendMessage("§4§lGrieferWeit §8» §7Du hast den Spieler §e" + CoinSystem.getInstance().getCoinAPI().getCoins(player.getUniqueId()) + "€ §8gegeben");
                        break;
                    case "remove":
                        CoinSystem.getInstance().getCoinAPI().removeCoins(Bukkit.getPlayer(args[1]).getUniqueId(), Integer.parseInt(args[2]));
                        player.sendMessage("§4§lGrieferWeit §8» §7Du hast den Spieler §e" + CoinSystem.getInstance().getCoinAPI().getCoins(player.getUniqueId()) + "€ §8entfernt");
                        break;
                    case "set":
                        CoinSystem.getInstance().getCoinAPI().setCoins(Bukkit.getPlayer(args[1]).getUniqueId(), Integer.parseInt(args[2]));
                        player.sendMessage("§4§lGrieferWeit §8» §7Du hast den Spieler auf §e" + CoinSystem.getInstance().getCoinAPI().getCoins(player.getUniqueId()) + "€ §8gesetzt");
                        break;
                    default:
